...
import com.CybaricFox.Modules.ArchTransport.ConveyorComponent;
import com.CybaricFox.Modules.ArchTransport.ConveyorInstance;
import com.CybaricFox.Modules.ArchTransport.ConveyorType;
...
public class ConveyorRefSystem extends RefSystem<ChunkStore> {
    ...
    public void onEntityAdded(@Nonnull Ref<ChunkStore> ref, @Nonnull AddReason addReason, @Nonnull Store<ChunkStore> store, @Nonnull CommandBuffer<ChunkStore> commandBuffer) {
        ConveyorComponent conveyorComponent = commandBuffer.getComponent(ref, ConveyorComponent.getComponentType());
    ...
    public Query<ChunkStore> getQuery() {
        return Query.or(ConveyorComponent.getComponentType());
    }
    ...
    public void changeTickState(Vector3i pos, CommandBuffer<ChunkStore> commandBuffer, boolean ticking) {
        ...
        ConveyorComponent conveyorComponent = commandBuffer.getComponent(blockRef, ConveyorComponent.getComponentType());
        if(conveyorComponent == null) return;
